...
// These values are used to categorize files and are validated by
// the Silo service when creating files.
const (
	...
	// FileCategoryAttachment represents Files extracted from the GOBL invoice
	// under the attachments section.
	FileCategoryAttachment = "attachment"
)

// SiloFilesService implements the Invopop Silo API.
type SiloFilesService service

// SiloFile represents a file that was generated by one of the job's intents
// and is stored by the Silo service.
type SiloFile struct {
	// The UUID that identifies the file.
	ID string `json:"id" title:"ID"`
	// ISO date and time this file was created.
	...
	// Key used to identify the file within a silo entry.
	...
	Previous []*SiloFileVersion `json:"previous,omitempty" title:"Previous"`
}

// SiloFileVersion contains a single version of a previously uploaded
// file.
type SiloFileVersion struct {
...
type CreateSiloFile struct {
	...
	// Key used to identify the file idempotently within a silo entry
	...
	Embeddable bool `json:"embeddable,omitempty" title:"Embeddable"`
	// When true, do not generate publically accessible links to the file.
	Private bool `json:"private,omitempty" title:"Private"`
	// Any additional meta data about the file.
	Meta map[string]string `json:"meta,omitempty" title:"Meta"`
}
...
func (s *SiloFilesService) Download(ctx context.Context, entryID, id string) (io.ReadCloser, error) {
	if id == "" {
	...
	p := path.Join(siloBasePath, entriesPath, entryID, siloFilesPath, id)
